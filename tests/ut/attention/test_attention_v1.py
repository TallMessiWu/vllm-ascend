...
class TestC8MXFPQfaMetadataPlan(TestBase):
    ...
    def test_no_v_descale_reaches_the_metadata_op(self):
        # The delivery this backend targets dropped v_descale from the metadata
        # op. Sending it is a TypeError; the parameter is gone, not ignored.
        _, invocations = self._plans([{}] * self.LAYERS_PER_STEP)
        self.assertNotIn("v_descale", invocations[0][1])

    def test_the_new_delivery_attrs_reach_the_operator(self):
        # head_dim_v and is_grad_enabled arrived in the same change that removed
        # v_descale. C8_MXFP gives K and V the same per-head dim, so V's is this
        # backend's head_size.
        _, invocations = self._plans([{}] * self.LAYERS_PER_STEP)
        kwargs = invocations[0][1]
        self.assertEqual(kwargs["head_dim_v"], self.impl.head_size)
        self.assertFalse(kwargs["is_grad_enabled"])
