...
def _maybe_group_align(quant_method, layer, input_size, output_size, output_partition_sizes):
    ...
    if isinstance(layer, RowParallelLinear):
        ...
        # Merged/QKV column (multi-shard) and gathered output are out of scope.
        if len(output_partition_sizes) != 1 or getattr(layer, "gather_output", False):
            return None
        ...
        **extra_weight_attrs,
        ...
        if ga is not None:
            if ga["dim"] == "row":
                ...
                # NOTE: column + bias=True is out of scope -- bias is built by the
                # layer after create_weights with the native even split, so its
                # loader offset would be wrong. The target ViT MLP fc1 has bias=False.
