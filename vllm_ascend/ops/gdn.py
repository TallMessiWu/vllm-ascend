...
class AscendGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def _forward_core(
        ...
        # decode 阶段（spec-decode 的 conv1d update）使用 triton 实现，
        # 与 prefill 路径保留的 AscendC custom op 形成 prefill=AscendC / decode=triton 的混合方案。
        if spec_sequence_masks is not None:
        ...
        if attn_metadata.num_prefills > 0:
            ...
            # decode 阶段（non-spec decode 的 conv1d update）使用 triton 实现。
